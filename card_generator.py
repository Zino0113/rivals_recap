...
def create_player_card(nickname, roblox_avatar_url, metrics, badges, score, level, tier_image_name):
    ...
    ranks_dir = os.path.join(assets_dir, "ranks")
    ...
    if os.path.exists(star_path):
        try:
            ...
            draw.text((star_x + (star_size - lv_w)/2, star_y + (star_size - lv_h)/2 - 5), level_str, font=font_level, fill=COLOR_WHITE)
        except: pass
    ...
    def draw_score_line(y, label, val):
        ...
        draw.text((val_x, y - 5), f"{val:,} 점", font=font_score_val, fill=COLOR_SCORE)

    # [수정] 라벨 변경: Score -> 시즌1 마감 점수
    draw_score_line(score_start_y, "시즌1 마감 점수 : ", score)
    # 최고 점수는 현재 final_elo와 동일하게 표시 (데이터 부족 시)
    ...
    for i in range(4):
        ...
        if i < len(sub_badges):
            ...
            if os.path.exists(b_abs_path):
                try:
                    b_img = Image.open(b_abs_path).convert("RGBA")
                    b_img = b_img.resize((346, 346))
                    paste_x = bx + (box_w - 346) // 2
                    paste_y = by + 50
